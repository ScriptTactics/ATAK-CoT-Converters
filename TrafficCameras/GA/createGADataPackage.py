...
def generate_manifest_xml(contents):
    for county, uuid_list in folder_structure.items():    
        # Get the current working directory
        root_directory = os.getcwd()

        if county:
            # Construct the path to the folder
            folder_path = os.path.join(root_directory, str(county))
            os.chdir(folder_path)
        else:
            county = "ungrouped"
        ...
        ET.SubElement(configuration, "Parameter", name="name", value=f"{county}_traffic_cams")
...
geo_json_path = "GDOT_Live_Traffic_Cameras.geojson"
...
for camera in cameras["features"]:
    ...
    if location_element is not None:
        ...
        region = camera['properties']['county']
